...
public class NavController extends View {
    ...
    private IMainPresenter.IMainView mView;

    public interface OnNavAndSpeedListener {
        void onNavAndSpeed(int nav);
        void onTouchDown();
    }
...
//            360-0  第四象限
            if (angle <= 0.3827) {
                //右
                if (mCallBack != null) {
                    mCallBack.onNavAndSpeed(5);
                }
            } else if (angle > 0.3827 && angle <= 0.9239) {
                //右下
                if (mCallBack != null) {
                    mCallBack.onNavAndSpeed(4);
                }
            } else if (angle > 0.9239) {
                //下
                if (mCallBack != null) {
                    mCallBack.onNavAndSpeed(3);
                }
            }
        } else if (aX >= 0 && aY < 0) {
            //0-90 第一象限
            if (angle <= 0.3827) {
                ...
                //右上
                if (mCallBack != null) {
                    mCallBack.onNavAndSpeed(6);
                }
            } else if (angle > 0.9239) {
                //上
                if (mCallBack != null) {
                    mCallBack.onNavAndSpeed(7);
                }
            }
        } else if (aX < 0 && aY >= 0) {
            //180-270   第三象限
            if (angle <= 0.3827) {
                //左
                if (mCallBack != null) {
                    mCallBack.onNavAndSpeed(1);
                }
            } else if (angle > 0.3827 && angle <= 0.9239) {
                //左下
                if (mCallBack != null) {
                    mCallBack.onNavAndSpeed(2);
                }
            } else if (angle > 0.9239) {
                //下
                if (mCallBack != null) {
